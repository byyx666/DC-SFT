...
class ExactAccuracy(ORM):

    def __call__(self, completions, solution, **kwargs) -> List[float]:
        rewards = []
        for content, sol in zip(completions, solution):
            gold_parsed=sol.strip().strip('.').lower()
            answer_parsed=content.strip().strip('.').lower()
...
class IOUAccuracy(ORM):
    ...
    def __call__(self, completions, solution, **kwargs) -> List[float]:
        ...
        for content, sol in zip(completions, solution):
            ...
            content_match = re.search(bbox_pattern, content)
            sol_match = re.search(bbox_pattern, sol)
            
            if content_match:
                bbox_content = [float(content_match.group(1)), float(content_match.group(2)), float(content_match.group(3)), float(content_match.group(4))]
                bbox_sol = [float(sol_match.group(1)), float(sol_match.group(2)), float(sol_match.group(3)), float(sol_match.group(4))]
                # reward = iou(bbox_content, bbox_sol)  
                if iou(bbox_content, bbox_sol)>=0.5:
                    reward=1
                else:
                    reward=0
            rewards.append(reward)
...
orms['external_exact_acc'] = ExactAccuracy
orms['external_iou_acc'] = IOUAccuracy
